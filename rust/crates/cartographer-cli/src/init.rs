...
const GITIGNORE_COMMENT: &str = "# Cartographer local artifacts";
...
pub(crate) fn render_init_claude_md(cwd: &Path) -> String {
    ...
    let mut lines = vec![
        "# CLAUDE.md".to_string(),
        String::new(),
        "This file provides guidance to Cartographer when working with code in this repository."
            .to_string(),
        String::new(),
    ];
...
mod tests {
    ...
    fn temp_dir() -> std::path::PathBuf {
        ...
        std::env::temp_dir().join(format!("cartographer-init-{}-{id}", std::process::id()))
    }
